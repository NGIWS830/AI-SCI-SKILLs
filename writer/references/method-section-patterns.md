...
This is the **preferred** simplified structure. Depart from it only when the paper type demands otherwise.
...
[开头段落：3-5 句话介绍整体框架。描述输入→处理流程→输出，
 点明三个创新点分别解决什么问题、它们之间如何协作。
 不设 "Overview" 小标题，直接写。]

### <创新点1名称>

[该创新点的具体内容：动机 → 设计 → 公式/机制 → 作用]
...
1. **大标题就是"方法"**，不需要" Proposed Method"或"Approach"等修饰。
2. **开头不加 Overview 小标题**。在"方法"大标题下直接写 3-5 句总述，交代整体框架和三个创新点的分工。
3. **三个创新点 = 三个小标题**。每个小标题是创新点的名称（如"方向感知特征编码""跨模态对齐模块""自适应融合策略"），不是"模块A""模块B"这种占位名。
4. **每个小标题下自成一段完整内容**：先说动机（为什么需要这个模块），再说设计（怎么做的，公式/图示），最后说作用（这个模块在整体框架中的位置和贡献）。
5. **损失函数和训练细节**的处理：
   - 如果损失函数是一个独立创新点，给它一个小标题。
   - 如果损失函数是某个模块的配套设计，放在该模块小标题内。
   - 训练细节（优化器、学习率、数据增强等）放在最后一个创新点之后，用 `### 实现细节` 或 `### 训练策略` 收尾。

### Opening Paragraph Template

> [方法名称]的整体架构如图[X]所示。给定[输入描述]，该方法首先通过[创新点1模块]对[输入]进行[操作1]，得到[中间结果A]，用于[目的1]。随后，[创新点2模块]利用[中间结果A]进行[操作2]，产生[中间结果B]，以[目的2]。最后，[创新点3模块]通过[操作3]将[中间结果B]转化为[最终输出]，实现[目的3]。三个模块协同工作，共同[核心目标一句话]。

### Per-Innovation Content Template

每个创新点小标题下的内容，按 **动机 → 设计 → 公式/机制 → 作用** 四段式展开。
中间可根据需要穿插模块框架图、其他示意图、或算法伪代码流程。

```
### <创新点名称>

[1-2 句动机] [现有方法的不足或观察到的现象]，为此设计了[该创新点]。

[2-4 句设计] 该模块的输入为 [X] ∈ R^{[dims]}。首先通过 [操作1] 对输入进行
[处理]，随后 [操作2] 产生 [中间结果]。最后 [操作3] 输出 [Y]，传递给[下游]。

[可选：模块框架图] 如图[X]所示，[模块名称]由[N]个子组件构成：[...]。
                   图中[颜色/箭头/标注]表示了[关键信息]。

[可选：算法伪代码] 算法 1 总结了[模块名称]的完整计算流程。
                   伪代码放 LaTeX 的 algorithm 环境，不在正文中逐行解释。

[公式/机制] [如有核心公式，用 display equation 呈现；每个符号在文中解释。]
            [如公式较多，按子步骤分段给出，每段先直觉后公式。]

[1-2 句作用] 该设计使得 [能力提升]，直接支撑 [贡献声明]，其有效性将在
第 IV-C 节消融实验中验证（表[X]）。
```

**四段式的顺序是固定的（动机→设计→公式/机制→作用）**，但图和伪代码的插入位置根据内容需要灵活放置：
- 模块框架图通常放在设计段之后，帮助读者建立空间结构认知。
- 算法伪代码通常放在公式/机制段之后，作为计算流程的形式化总结。
- 其他示意图（如特征可视化、数据流对比）放在与之相关的段落附近。
