...
| **Abstract** | 2-6-2 结构，恰好 10 句：2 句背景/缺口 + 6 句方法（用 First/Second/Finally 串联，每对 = 声明+为什么有效）+ 2 句结果/意义。~250 汉字 |
...
| **Method** | III-A 总览（附架构图引用），III-B/C/D 每模块：输入→处理→输出→设计理由。每个公式必须在正文中解释 |
| **Experiments** | 子节：数据集与实现细节 / 主要结果 / 消融研究 / 效率分析 / 定性分析。每个结果数字必须同时在表格和正文中出现 |
| **Conclusion** | 重述问题、方法、关键发现。不得添加新声明或引用。结尾 1-2 个具体未来方向 |
