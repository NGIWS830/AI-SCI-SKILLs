...
To address [problem] in [RS task], we propose [Method]. [Method] consists of
three core components: [Innovation 1], which [purpose]; [Innovation 2], which
[purpose]; and [Innovation 3], which [purpose]. The main contributions of this
paper are as follows:

\begin{enumerate}
  \item [Innovation 1 contribution]. We design [Module A], which [mechanism]
        tailored for [RS data characteristic] to achieve [capability]
        (Section~\ref{sec:method-innov1}).
  \item [Innovation 2 contribution]. We propose [Module B], which [mechanism]
        to [purpose], addressing [RS challenge] (Section~\ref{sec:method-innov2}).
  \item [Innovation 3 contribution]. We introduce [Module C], which [mechanism]
        to [purpose]. Extensive experiments on [N] RS datasets demonstrate
        [key result] (Section~\ref{sec:experiments}). Code is available.
\end{enumerate}
